...
face_detector = YOLO('yolov8n-face.pt')  #my changes
...
def detect_faces(image_np):
    ...
    Output: list of (x, y, width, height) of faces
    """
    results = face_detector.predict(source=image_np, conf=0.5, verbose=False)[0]
    detections = []
...
class NeuralHash:
    ...
    def im2array(self, image_path):
        ...
        detections = detect_faces(img_np)  # Call detect_faces globally

        if detections:
            x, y, width, height = detections[0]  # Pick the first face
            x, y = max(0, x), max(0, y)
